...
public class ArenaManager {
  ...
  /**
   * Get a list of active settings for a position. If two ArenaSetting's priorities conflict, the first one that the
   * computer finds is chosen.
   *
  ...
  /**
   * Get a list of active settings for a player. If two ArenaSetting's priorities conflict, the first one that the
   * computer finds is chosen.
   *
   * @param player GamePlayer
  ...
  public static ArenaSettings getActiveSettings(GamePlayer player) {
    ...
    if (player.getAreas().size() == 0) {
      return player.getGame().getArena().getArenaSettings();
    }
  ...
  public static List<ArenaSettings> getArenaSettingsByLocation(Location location) {
    ...
    for (Game game : GameManager.getGames()) {
      if (game.getArena().getWorld().getName().equals(location.getWorld().getName())) {
        settings.add(game.getArena().getArenaSettings());
      }
